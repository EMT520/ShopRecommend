...
/*
yarn jar first-1.0-SNAPSHOT.jar com.briup.MR.InputFormat.HdfsToDb -D input=/user/hdfs/db.txt
 */
//写入数据库使用Dboutputformat，写入数据库reduce的key为写入对象
//如果没有reduce，那么map输出的键是写入对象
public class HdfsToDb extends Configured implements Tool {
    ...
    static class HdfsToDbMapper extends Mapper<LongWritable, Text,Teacher, NullWritable> {
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            Teacher tea=new Teacher(Long.parseLong(strs[0]),
                    strs[1],Integer.parseInt(strs[2]));
            context.write(tea,NullWritable.get());
        }
    }
    static class HdfsToDbReduce
            extends Reducer<Teacher,NullWritable,Teacher,NullWritable>{
        @Override
